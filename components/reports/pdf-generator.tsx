...
export function generatePDFReport({
  ...
  doc.text(`Tanggal Cetak: ${formatDate(new Date())}`, 105, 42, {
    align: "center",
  });
  ...
  doc.text(`Rp ${formatCurrency(grossAmount)}`, rightCol, rowY, {
    align: "center",
  });
  rowY += rowHeight;
  ...
  doc.text(`Rp ${formatCurrency(totalDiscount)}`, rightCol, rowY, {
    align: "center",
  });
  rowY += rowHeight;
  ...
  doc.text(`Rp ${formatCurrency(totalAmount)}`, rightCol, rowY, {
    align: "center",
  });
  rowY += rowHeight;
  ...
  doc.text(`Rp ${formatCurrency(avgPerTransaction)}`, rightCol, rowY, {
    align: "center",
  });
  rowY += rowHeight;
  ...
  if (transactions.length === 0) {
    doc.text("Tidak ada transaksi pada bulan yang dipilih", 105, yPos, {
      align: "center",
    });
  } else {
  ...
  doc.setFontSize(10);
  doc.text(`Laporan ini dibuat secara otomatis oleh sistem POS`, 105, yPos, {
    align: "center",
  });
  yPos += 7;
  doc.text(
